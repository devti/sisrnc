...
public class ProcuraPessoaGUI extends JDialog {
	...
	public void procurar() throws RepositorioException {
		try {			
			...
			lista = fachada.pessoaPesquisaAvancada(cbAtributo.getSelectedItem().toString(), tfPesquisa.getText());
			ptm = new PessoaTableModel(lista);
			table.setModel(ptm);
	...
	private void ok() {
		try {
			if (table.getSelectedRow() != -1) {
				fachada = Fachada.getInstance();
				int codPessoa = (int) ptm.getValueAt(table.getSelectedRow(), 0);
				pessoa = fachada.pessoaProcurar(codPessoa);
				dispose();
			} else {
